...
const ResetPassword = () => {
  ...
  const onSubmitOtp = async (e) => {
    ...
    // try {
    //   const { data } = await axios.post(backend_url + "/api/auth/verify-reset-otp", { email, otp });
    //   data.success ? toast.success(data.message) : toast.error(data.message);
    //   if (data.success) setIsOtpSub(true);
    // } catch (error) {
    //   toast.error(error.message);
    // }
  };
  ...
  const onSubmitPassword = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post(backend_url + "/api/auth/reset-pass", {
        email,
        newpassword,
        otp,
      });
